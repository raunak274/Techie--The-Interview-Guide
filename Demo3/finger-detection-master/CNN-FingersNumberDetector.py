...
class FingersNumberDetector:
    ...
    def startDetecting(self):
        ...
        while cap.isOpened():
            ...
            cv2.rectangle(frame, (self.x0, self.y0), (self.x0 + self.width - 1, self.y0 + self.height - 1),
                          self.roiColor, 2)

            cv2.putText(frame, 'Sampling data: {}'.format(self.sampleData), (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                        self.roiColor, 2, 1)
            cv2.putText(frame, 'Class: {} {} ({})'.format(self.currentClass, self.sampleCategory, self.instanceCount),
                        (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, self.roiColor, 2, 1)
